...
export default function Stepper({
  ...
  const { className: externalClassName, ...restProps } = rest;

  return (
    <div
      className={`flex min-h-full flex-1 flex-col items-center justify-center p-4 ${externalClassName ?? ""}`}
      {...restProps}
    >
      <div
          ...
          <div className={`px-8 pb-8 ${footerClassName}`}>
            ...
            >
              ...
              <button
                ...
                {...nextButtonProps}
              >
                {isLastStep ? (finalStepButtonText ?? "Complete") : nextButtonText}
              </button>
            </div>
          </div>
        )}
    ...
    >
      <motion.div
        variants={{
          ...
          active: {
            scale: 1,
            backgroundColor: "var(--tomato, #ff6347)",
...
function StepConnector({ isComplete }: StepConnectorProps) {
  const lineVariants: Variants = {
    ...
    complete: { width: "100%", backgroundColor: "var(--tomato, #ff6347)" },
  };
