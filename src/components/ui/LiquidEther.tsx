...
`;
    ...
    class ShaderPass {
      ...
      constructor(props: Record<string, unknown>) {
        this.props = props || {};
        this.uniforms = (
          this.props.material as { uniforms?: Uniforms } | undefined
        )?.uniforms;
      }
      init(..._args: unknown[]) {
        void _args;
    ...
    class Advection extends ShaderPass {
      ...
      createBoundary() {
        ...
        const boundaryM = new THREE.RawShaderMaterial({
          ...
          fragmentShader: advection_frag,
          uniforms: this.uniforms as Record<
            string,
      ...
      update(...args: unknown[]) {
        const { dt: dtVal, isBounce, BFECC } = (args[0] || {}) as {
          dt?: number;
          isBounce?: boolean;
          BFECC?: boolean;
        };
        if (!this.uniforms) return;
        if (typeof dtVal === "number") this.uniforms.dt.value = dtVal;
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: unknown[]) {
        ...
        const forceX =
          (Mouse.diff.x / 2) * ((props.mouse_force as number) || 0);
        const forceY =
          (Mouse.diff.y / 2) * ((props.mouse_force as number) || 0);
        const cellScale = (props.cellScale as { x: number; y: number }) || {
          x: 1,
          y: 1,
        };
        const cursorSizeVal = (props.cursor_size as number) || 0;
        const cursorSizeX = cursorSizeVal * cellScale.x;
        const cursorSizeY = cursorSizeVal * cellScale.y;
        ...
        uniforms.scale.value.set(cursorSizeVal, cursorSizeVal);
        super.update();
      }
    }
    ...
    class Viscous extends ShaderPass {
      ...
      update(...args: unknown[]) {
        const { viscous: viscousVal, iterations, dt: dtVal } = (args[0] ||
          {}) as {
          viscous?: number;
          iterations?: number;
          dt?: number;
        ...
        if (typeof viscousVal === "number") this.uniforms.v.value = viscousVal;
    ...
    class Pressure extends ShaderPass {
      ...
      update(...args: unknown[]) {
        const { vel, pressure: pressureVal } = (args[0] || {}) as {
          vel?: THREE.WebGLRenderTarget;
          pressure?: THREE.WebGLRenderTarget;
        };
        if (this.uniforms && vel && pressureVal) {
          this.uniforms.velocity.value = vel.texture;
          this.uniforms.pressure.value = pressureVal.texture;
        }
        super.update();
      }
    }
    ...
    class Simulation {
      ...
      update() {
        ...
        let vel: THREE.WebGLRenderTarget | undefined = this.fbos
          .vel_1 as THREE.WebGLRenderTarget;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: Record<string, unknown>) {
        ...
        this.autoDriver = new AutoDriver(Mouse, this as WebGLManager, {
