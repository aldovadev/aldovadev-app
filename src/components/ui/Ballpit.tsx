...
class ThreeInstance {
  #config: ThreeConfig;
  #postprocessing: { render: (...args: unknown[]) => void; setSize: (w: number, h: number) => void; dispose?: () => void } | null = null;
  ...
  #initRenderer() {
    ...
    if (!this.canvas) {
      throw new Error('Ballpit requires a canvas element.');
    }
  ...
  set postprocessing(value: { render: (...args: unknown[]) => void; setSize: (w: number, h: number) => void; dispose?: () => void } | null) {
...
class SSSMaterial extends MeshPhysicalMaterial {
  ...
  onBeforeCompile2?: (shader: { uniforms: Record<string, unknown>; vertexShader: string; fragmentShader: string }) => void;
}
...
class Spheres extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<typeof defaultBallpitConfig> = {}) {
    ...
    const material = new SSSMaterial({ envMap: envTexture, ...config.materialParams });
      ...
      const colorUtils = (function (colorsArr: number[]) {
        ...
        return {
          getColorAt: (ratio: number, out: Color = new Color()) => {
            ...
            const scaled = clamped * (colorsArr.length - 1);
  ...
  update(deltaInfo: { delta: number }) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      tempObj.position.fromArray(this.physics.positionData, 3 * idx);
      if (idx === 0 && this.config.followCursor === false) {
        tempObj.scale.setScalar(0);
      } else {
        tempObj.scale.setScalar(this.physics.sizeData[idx]);
      }
      tempObj.updateMatrix();
      this.setMatrixAt(idx, tempObj.matrix);
      if (idx === 0) this.light!.position.copy(tempObj.position);
    }
...
function createBallpit(canvas: HTMLCanvasElement, config: Partial<typeof defaultBallpitConfig> & Record<string, unknown> = {}): CreateBallpitReturn {
  ...
  function initialize(cfg: Partial<typeof defaultBallpitConfig> & Record<string, unknown>) {
    if (spheres) {
      threeInstance.clear();
...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, ...props }) => {
  ...
  const configRef = useRef<Partial<typeof defaultBallpitConfig> & Record<string, unknown>>({
    followCursor,
    ...props
  });

  useEffect(() => {
    configRef.current = { followCursor, ...props };
  }, [followCursor, props]);
